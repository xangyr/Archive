...
// FETCH STATE = PC
                        // The PC
PC32 thePC(.in(NextPC),.out(PC),.en(NoHazardDetected),.reset(1'b0),.clk(clk));
...
assign NextInst = (IFlush)?NOPbits:IBits;

// IF-ID Pipeline Register
IF_ID_PR thisIF_ID_PR(.clk(clk),.en(NoHazardDetected),.BeingFetched(NextInst),.WasFetched(ID_IBits),.PCp4(PCp4),.ID_PCp4(ID_PCp4));
...
ID_EX_PR thisID_EX_PR(.clk(clk),.en(1'b1),
...
EX_MEM_PR thisEX_MEM_PR(.clk(clk),.en(1'b1),
...
.MemOut_in(MemOut),
.PCp4_in(MEM_PCp4),
.memToReg_in(MEM_memToReg_ctrl),
.Link_in(MEM_Link_ctrl),
.RegWrite_in(MEM_regWrite_ctrl),
.SYSCALL_in(MEM_SYSCALL_ctrl),
.RegWriteDest_in(MEM_RegWriteDest),
.WB_ALUout(WB_ALUOut),
.WB_MemOut(WB_MemOut),
.WB_PCp4(WB_PCp4),
